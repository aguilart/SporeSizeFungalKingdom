...
#                                 "phylum","class","order")])
trial<-left_join(dat %>% 
                  select(names_to_use,host.assoc, ends_with('mean')), 
                 To_Analysis[,c("names_to_use","SporeType","simpleFunct","SporeVolume", "Q_ratio",
                                "phylum","class","order")])
...
# global model
m0 <- lm(log10(SporeVolume) ~ 1, data=trial)
m1 <- lm(log10(SporeVolume) ~ order + simpleFunct + SporeType + 
             mat_mean + map_mean + maxsrad_mean + minvapr_mean, data=trial)
m1.drp <- drop1(m1) %>% 
  rownames_to_column('term') %>% 
  mutate(dAIC = AIC - AIC[1])
m1.add <- add1(m0, formula(m1))

m2 <- lm(log10(SporeVolume) ~ order + host.assoc + SporeType + 
             mat_mean + map_mean + maxsrad_mean + minvapr_mean, data=trial)
drop1(m2)
summary(m1)

# global model
m3 <- lm(log10(Q_ratio) ~ order + simpleFunct + SporeType + 
             mat_mean + map_mean + maxsrad_mean + minvapr_mean, data=trial)
drop1(m3)
