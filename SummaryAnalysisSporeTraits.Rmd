...
ape::write.tree(Li_tree, "C:\\Users\\Carlos\\Documents\\Professional\\SporeSizeAcrossFungKingdom\\Manuscript_submission_versions\\PNAS\\Review\\Supplementary_data\\Li_tree.txt")


ape::write.tree(tax_tree2, "C:\\Users\\Carlos\\Documents\\Professional\\SporeSizeAcrossFungKingdom\\Manuscript_submission_versions\\PNAS\\Review\\Supplementary_data\\tax_tree2.txt")
...
source("analysis_forNM_March23.R")

# Fix the entries with double "Plant pathogen" and also the two categories for necrotrophs
...
Simplifying the figure for Ecology Letters

```{r}
temp2 <- temp

temp2$simpleFunct <- c("#44AA99",rep("#882255", 9))

predicted_ascos_basidios$Lifestyle <- predicted_ascos_basidios$simpleFunct
predicted_ascos_basidios$Lifestyle[grep("Asymbiotic",
                                   predicted_ascos_basidios$simpleFunct)] <- "Asymbiotic fungi"
predicted_ascos_basidios$Lifestyle[-grep("Asymbiotic",
                                   predicted_ascos_basidios$simpleFunct)] <- "Symbiotic fungi"
#
predicted_mitos$Lifestyle <- predicted_mitos$simpleFunct
predicted_mitos$Lifestyle[grep("Asymbiotic",
                                   predicted_mitos$simpleFunct)] <- "Asymbiotic fungi"
predicted_mitos$Lifestyle[-grep("Asymbiotic",
                                   predicted_mitos$simpleFunct)] <- "Symbiotic fungi"

pdf("Figures/Figure_2/EL_Figure2.pdf",
    width = 23,
    height = 28)
cowplot::plot_grid(
coloreando(predicted_ascos_basidios,
           temp2) %>% 
  ggplot() +
  geom_hline(yintercept = 10^2.08, color = "#44AA99",
             linetype = "solid", linewidth = 2, alpha = 0.2) +
  geom_hline(yintercept = 10^2.24, color = "#44AA99",
             linetype = "solid", linewidth = 2, alpha = 0.2) +
  geom_pointrange(aes(x = simpleFunct, y = 10^Estimate,
                      ymin = 10^lower, ymax = 10^upper,
                      color = colores), size = 2) + 
...
coloreando(predicted_mitos,
           temp2) %>% 
  filter(grepl("Asco|Basidio", phylum)) %>% 
  ggplot() +
  geom_hline(yintercept = 10^1.67, color = "#44AA99",
             linetype = "solid", linewidth = 2, alpha = 0.2) +
  geom_hline(yintercept = 10^1.73, color = "#44AA99",
             linetype = "solid", linewidth = 2, alpha = 0.2) +
  geom_pointrange(aes(x = simpleFunct, y = 10^Estimate,
                      ymin = 10^lower, ymax = 10^upper,
                      color = colores), size = 2) +
  ...
  filter(!grepl("Asco|Basidio", phylum)) %>% 
  ggplot() +
  geom_hline(yintercept = 10^2.41, color = "#44AA99",
             linetype = "solid", linewidth = 2, alpha = 0.2) +
  geom_pointrange(aes(x = simpleFunct, y = 10^Estimate,
                      ymin = 10^lower, ymax = 10^upper,
                      color = colores), size = 2) +
  ...
  scale_color_identity() +
  facet_wrap(phylum~., scales = "free_x")+ #, scales = "free_y") +
  scale_y_log10(breaks = c(10^2,10^2.5,10^3,10^3.5),
                labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  theme_1,
  ncol = 1, align = "v",
...
ggsave(filename="Figures\\Figure_2\\EL_Figure2.svg",
...
coloreando(predicted_mitos,
  ...
  coloreando(bind_rows(predicted_mitos_Li[grep("Micro|Zoo", predicted_mitos_Li$phylum), ],
                       predicted_mitos[grep("Zygo", predicted_mitos$phylum), ]),
           temp2) %>% 
  filter(!grepl("Asco|Basidio", phylum)) %>% 
  ggplot() +
  geom_hline(yintercept = 10^1.83, color = "#44AA99",
             linetype = "solid", linewidth = 2, alpha = 0.2) +
  geom_hline(yintercept = 10^2.41, color = "#44AA99",
             linetype = "solid", linewidth = 2, alpha = 0.2) +
  geom_pointrange(aes(x = simpleFunct, y = 10^Estimate,
  ...
  ncol = 1, align = "v",
rel_heights = c(1,1,0.7), labels = "auto", label_size = 35),
height = 28, width = 21,device="svg")
```

Figure S3

```{r}
pdf("Figures/Figure_2/FigureS3_bubbleplot.pdf",
    width = 23,
    height = 28)
cowplot::plot_grid(
...
leyenda <- 
  temp %>% 
  mutate(dumb_y = rep(c(2,1), c(5,5)),
         dumb_x = c(1:5,1:5)) %>% 
  mutate(names = gsub("\\d+_|\\d+\\.\\d_", "", names)) %>% 
  mutate(names = gsub("\\n", " ", names)) %>% 
...
pdf("Figures/Figure_2/EL_FigureS4_bubbleplot.pdf",
    width = 23,
    height = 28)
cowplot::plot_grid(
