...
m1.drp <- as.data.frame(drop1(m1)) %>% 
  ...
  ungroup() %>% 
  select(term, Df, AIC, dAIC)
m1.add <- vector('list', length=nrow(m1.drp))
names(m1.add) <- m1.drp$term
m1.add[[1]] <- m1$model
for(i in names(m1.add)[2:length(m1.add)]) m1.add[[i]] <- dplyr::select(m1$model, -matches(i))
m1.drp <- m1.drp %>% 
  mutate(AdjR2 = sapply(m1.add, function(x)summary(lm(log10.sporeVolume ~ ., data=x))[['adj.r.squared']]), 
...
for(i in names(m1.add)[2:length(m1.add)]) m1.add[[i]] <- dplyr::select(m1$model, -matches(i))
m1.drp <- m1.drp %>% 
  mutate(AdjR2 = sapply(m1.add, function(x)summary(lm(log10.sporeAspectRatio ~ ., data=x))[['adj.r.squared']]), 
         dAdjR2 = AdjR2 - AdjR2[1])
