...
# pdf("trial.pdf", width = 18, height = 70)
# plot.phylo(tax_tree2, cex = 0.1)
# dev.off()
```
...
tablas <- 
rbind(as.matrix(Li_meios1), matrix(names(Li_meios2), nrow = 1, ncol = 4), as.matrix(Li_meios2),
...
write.table(tablas,
          "model_outputs/simple_funct_models.txt", sep =";",
          row.names = F)
```
...
# This is the version done with the Lutzoni tree (written by Will)
...
write.tree(tt,"trial_tree.phy")

tt2 <- read.tree("trial_tree2.phy.txt")
...
# First exclude remove the genera that are in the Lutzoni tree but are not present in the
# spore dataset (which comes from orders dataframe)

nnn<-tt$tip.label
tt$tip.label<-split(nnn)
dropped.tree<-drop.tip(tt,tt$tip.label[which(!tt$tip.label %in% only_one_genera_per_order$genus)])

# Second remove the duplicated genera per oder in the tree (really to have just one genera
# per order)
...
# Third replace the names of the genus with the name of the orders
...
plot.phylo(dropped.tree2, use.edge.length = F, cex = 0.3)
...
# Root in the tree
tt <- root(tt, outgroup = outgroups); rm(Li_tree2)
...
paso1<-
Spore_functions[which(Spore_functions$order%in%dropped.tree2$tip.label),c("phylum","order")]
...
#paso3$order<-factor(paso3$order, levels = with(d, label[order(y, decreasing=T)]))
...
old
```{r}
g <- match(en_orden, paso3$order)
all(paso3[g, ]$order == en_orden) #This is true

paso3 <- paso3[g, ]; rownames(paso3) <- NULL

# It has to go in the opposite order
g <- (as.numeric(rownames(paso3))-86)*-1
paso3 <- paso3[g, ]
all(paso3$order == en_orden[g]) #This is also true

paso3$order<-factor(paso3$order, levels = en_orden[g])
...
#paso3$order <- as.factor(paso3$order)

paso3 %>% 
  mutate(Life_style=factor(Life_style,
                           levels = c("Obligate_association",
                                       "Facultative_association",
                                       "AFree_living"))) %>% 
  ggplot()+
...
Old
Counting how many species per order we have
```{r}
...
AllFungi[which(AllFungi$order%in%unique(paso3$order)),c("names_to_use","order")] %>% 
...
#"Figures/Figure_1/Figure1_heatmaps_vol_cv.pdf", and dimensions:
#   width = 16,
#    height = 19)

heatmap(datos2,Colv = NA, Rowv = NA,cexCol = 0.5,cexRow = 0.5,scale = "none",labRow = F, labCol = F,
        col=coul)
legend(x="bottomright", legend=c("a", "b", "c","d","e"), 
...
# supp.labs <- c("Aspect ratio L/W", expression("Spore Volume ("*mu*"m³)"))
# names(supp.labs) <- c("Q_ratio", "SporeVolume")
# 
# new_labels <- c("Q_ratio"= "Aspect ratio (L/W)", "SporeVolume"= expression("Spore Volume ("*mu*"m³)"))
...
# tapply(log10(To_Analysis_c$SporeVolume[which(To_Analysis_c$SporeType=="Meiospores")]),
#         list(To_Analysis_c$Life_style[which(To_Analysis_c$SporeType=="Meiospores")],
#              To_Analysis_c$phylum[which(To_Analysis_c$SporeType=="Meiospores")]),
#        mean)

my_theme_ridges<-theme(title = element_text(size = 18),
        #plot.background = element_rect(fill="gray95"),
        #panel.background = element_rect(fill = "gray95"),
        panel.background = element_blank(),
        
        panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_line(size = 4, #linetype = 'longdash',
                                        colour = "gray"),
        axis.title.y=element_blank(),
        axis.title.x = element_blank(),
...
To_Analysis_c %>% 
  ...
  filter(SporeType=="Meiospores") %>% 
  filter(!(phylum=="Basidiomycota"&simpleFunct=="Human")) %>% 
  filter(!(phylum=="Basidiomycota"&SporeVolume>10000)) %>% #One species (which I checked) has a very high volume, because ridges function extend the tail with a color because of only this point I am removing it
  mutate(Life_style = recode(Life_style, Plant = "Facultative_association")) %>% 
  mutate(Life_style = recode(Life_style, B_Opportunistic_association = "Asymbiotic")) %>% 
  mutate(Life_style = recode(Life_style, AFree_living = "Asymbiotic")) %>%
  mutate(Life_style = recode(Life_style, Facultative_association = "Facultative symbiosis")) %>% 
  mutate(Life_style = recode(Life_style, Obligate_association = "Obligate symbiosis")) %>%
...
geom_density_ridges(jittered_points = TRUE, #position = "raincloud",
    scale = 1.8,rel_min_height = 0.01,point_color="white",size=2,alpha=0.9,point_size=3,
    quantile_lines=TRUE,quantile_fun=function(x,...)mean(x))+
  scale_fill_manual(values= c("#FDD074","#F58345","#E43339","#AA2327"))+
  scale_color_manual(values= c("#FDD074","#F58345","#E43339","#AA2327"))+
  
  scale_x_log10(breaks=c(10^0,10^1,10^2,10^3,10^4,10^5,10^6),
                labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  facet_wrap(.~phylum, strip.position =  "top",nrow = 1,drop=FALSE,scales="free_x")+
...
To_Analysis_c %>% 
  ...
  aes(y=Life_style,
      ...
      fill=Life_style)+
geom_density_ridges(jittered_points = TRUE, #position = "raincloud",
    scale = 1.8,rel_min_height = 0.001,point_color="white",size=2,alpha=0.9,point_size=3,
    quantile_lines=TRUE,quantile_fun=function(x,...)mean(x))+
  scale_fill_manual(values= c("#FDD074","#F58345","#E43339","#AA2327"))+
  scale_color_manual(values= c("#FDD074","#F58345","#E43339","#AA2327"))+
